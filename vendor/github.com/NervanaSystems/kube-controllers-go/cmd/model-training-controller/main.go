...
func main() {
	...
	// Create new CRD handle for the model training resource type.
	crdHandle := crd.New(
		&crv1.ModelTraining{},
	...
	if err != nil {
		...
		glog.Warningf("error while writing %s CRD in namespace %s: %s", crv1.ModelTrainingResourceKind, *namespace, err)
	}
