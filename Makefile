...
build: prereq dep-ensure code-generation lint test
	...
	go build -gcflags "-N -l" github.com/IntelAI/vck/pkg/validation-webhook
...
lint:
	...
	gometalinter --config=./lint.json ./pkg/validation-webhook...
...
test:
	...
	go test -v --cover ./pkg/validation-webhook...
